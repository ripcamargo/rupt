...
function syncPendingTasksWithApp() {
  ...
  chrome.runtime.sendMessage({ type: 'GET_PENDING_TASKS' }, (response) => {
    ...
    if (pendingTasks.length > 0) {
      console.log('[Rupt Extension Content Script] Syncing ' + pendingTasks.length + ' tasks with app');
      // Send immediately and once more after a delay to cover timing issues
      [0, 500].forEach((delay) => {
        setTimeout(() => {
          window.postMessage({
            source: 'rupt-extension-sync',
